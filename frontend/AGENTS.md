...
User-visible product text in this frontend uses **Semantic DeerFlow**. The UI is a
development and debugging interface for the downstream SaaS backend APIs, not a
finished standalone product frontend. Do not replace the existing Logo or favicon in
this release. Current-project GitHub and Issue links come from the optional
`NEXT_PUBLIC_REPOSITORY_URL`; hide those links when it is unset. ByteDance DeerFlow
links belong only in explicit upstream-source or acknowledgment copy.
