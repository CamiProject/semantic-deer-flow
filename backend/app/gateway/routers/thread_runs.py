...
@router.post("/{thread_id}/runs/saas-query/stream")
@require_permission("runs", "create", owner_check=True, require_existing=True)
async def stream_saas_query_run(
    thread_id: str,
...
@router.post("/{thread_id}/runs/saas-query/wait", response_model=dict)
@require_permission("runs", "create", owner_check=True, require_existing=True)
async def wait_saas_query_run(
    thread_id: str,
    body: RunCreateRequest,
    request: Request,
) -> dict:
