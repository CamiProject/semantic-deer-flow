...
test.describe("Landing page", () => {
  test("renders the header and hero section", async ({ page }) => {
    ...
    await expect(
      page
        .locator("header")
        .first()
        .getByText("Semantic DeerFlow", { exact: true }),
    ).toBeVisible();
    await expect(page.locator("h1")).toHaveCount(1);
    await expect(page.locator("h1")).toContainText("Semantic DeerFlow");

    // "Get Started" call-to-action button in hero
    await expect(
      page.getByRole("link", { name: /open development workspace/i }),
    ).toBeVisible();
  });
  ...
  test("Get Started link navigates to workspace", async ({ page }) => {
    ...
    const getStarted = page.getByRole("link", {
      name: /open development workspace/i,
    });
